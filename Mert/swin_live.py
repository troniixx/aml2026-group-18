...
# ── config ────────────────────────────────────────────────────────────────────
CHECKPOINT   = "swin_best.pth"
MODEL_PATH   = "hand_landmarker.task"
CAMERA_INDEX = 1
DEVICE       = torch.device("cpu")  # MPS unreliable for Swin live inference
...
def build_swin(num_classes):
    m = models.swin_s(weights=None)
    in_feats = m.head.in_features
    m.head = nn.Sequential(
        nn.LayerNorm(in_feats), nn.Dropout(0.4),
        nn.Linear(in_feats, 256), nn.GELU(),
        nn.Dropout(0.3), nn.Linear(256, num_classes)
    )
    return m

model = build_swin(num_classes=len(SEAL_CLASSES))
...
while True:
    ...
    cv2.putText(frame, "Swin-S", (20, 135),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (150, 150, 150), 1)

    cv2.imshow("Naruto Handsign — Swin-S", frame)
